...
{options={};}
var new_slice=new Donatello.LineGraph.Slice(start,stop,this.paper,point1,point2,this.height,jQuery.extend(this.slice,options));new_slice.edge=this.addEdge(start,stop,point1,point2);this.slices.push(new_slice);return new_slice;};Donatello.LineGraph.prototype.addEdge=function(start,stop,point1,point2,options){if(options==null)
{options={};}
var new_edge=new Donatello.LineGraph.Edge(start,stop,this.paper,point1,point2,jQuery.extend(this.edge,options));this.edges.push(new_edge);return new_edge;};Donatello.LineGraph.prototype.addPoint=function(value,options){if(options==null)
{options={};}
var x=(((value[0]-this.min[0])/this.max[0])*this.width);var y=(((value[1]-this.min[1])/this.max[1])*this.height);var new_point=new Donatello.Point(value,this.paper,x,y,jQuery.extend(this.point,options));this.elements.push(new_point);return new_point;};Donatello.LineGraph.Slice=function(value1,value2,paper,start,stop,height,options){Donatello.Element.call(this,[value1,value2]);this.start=start;this.stop=stop;this.height=height;var edge_path=('M'+this.start[0]+','+this.start[1]+'L'+this.stop[0]+','+this.stop[1]);var full_path=(edge_path+
('M'+this.stop[0]+','+this.stop[1]+'V'+this.height)+
('M'+this.stop[0]+','+this.height+'H'+this.start[0])+
('M'+this.start[0]+','+this.height+'V'+this.start[1])+'Z');this.node=paper.path(full_path).attr({stroke:options.color,fill:options.color});if(options.opacity)
{this.setOpacity(options.opacity);}};Donatello.LineGraph.Slice.prototype=new Donatello.Element();Donatello.LineGraph.Slice.constructor=Donatello.LineGraph.Slice;Donatello.LineGraph.Edge=function(value1,value2,paper,start,stop,options){Donatello.Element.call(this,[value1,value2]);this.start=start;this.stop=stop;this.node=paper.path('M'+start[0]+','+start[1]+'L'+stop[0]+','+stop[1]);this.setColor(options.color);if(options.width!=null)
{this.node.attr('stroke-width',options.width);}};Donatello.LineGraph.Edge.prototype=new Donatello.Element();Donatello.LineGraph.Edge.constructor=Donatello.LineGraph.Edge;Donatello.LineGraph.BaseLine=function(value,paper,y,length,options){Donatello.Element.call(this,value);if(options==null)
{options={};}
...
this.piechart=(options.piechart||{});this.border=(options.border||{});if(this.edge.width==null)
{this.edge.width=0;}
